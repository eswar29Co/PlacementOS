...
export default function JobDetails() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Loading Spectrum" subtitle="Accessing simulation parameters...">
  ...
  if (!job) {
    return (
      <DashboardLayout title="Protocol Error" subtitle="The requested simulation node is offline.">
  ...
  const handleApply = () => {
    toast.success('Simulation protocols initialized. Synchronizing application data.');
    navigate(`/student/apply/${job.id}`);
  };

  return (
    <DashboardLayout title="Opportunity Dossier" subtitle={`Simulation Node: ${job.companyName}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-8 space-y-10">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10">
                  ...
                  <div className="flex-1 text-center md:text-left space-y-3">
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                      ...
                      <Badge className="bg-primary/5 text-primary border-primary/10 font-black px-4 py-1 rounded-full uppercase text-[10px] shadow-none">Prime Mission</Badge>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-3 pt-4">
                      ...
                      <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/5 text-amber-600 rounded-xl font-bold text-xs border border-amber-500/10">
                        <Zap className="h-3.5 w-3.5" /> Simulation Core
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <Card className="border-slate-200 shadow-sm rounded-[2.5rem] bg-white overflow-hidden border">
              <CardHeader className="bg-slate-50 p-8 border-b border-slate-100">
                <CardTitle className="text-xl font-black uppercase tracking-tight flex items-center gap-3 text-slate-900">
                  <Info className="h-6 w-6 text-primary" /> Tactical Briefing
                </CardTitle>
              </CardHeader>
              <CardContent className="p-10 space-y-10">
                <div className="space-y-4">
                  <h4 className="font-black text-xs uppercase tracking-[0.2em] text-slate-400">Mission Objective</h4>
                ...
                <div className="space-y-6">
                  <h4 className="font-black text-xs uppercase tracking-[0.2em] text-slate-400">Neural Specializations</h4>
            ...
            <Card className="border-slate-200 shadow-sm rounded-[2.5rem] bg-white overflow-hidden border">
              <CardHeader className="bg-slate-50 p-8 border-b border-slate-100">
                <CardTitle className="text-xl font-black uppercase tracking-tight flex items-center gap-3 text-slate-900">
                  <ShieldCheck className="h-6 w-6 text-emerald-500" /> Deployment Stages
                </CardTitle>
              </CardHeader>
              <CardContent className="p-10">
                <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
                  {['Resume Screening', 'Tactical Assessment', 'Neural AI Interview', 'Technical Calibration', 'Strategic Management', 'HR Compliance', 'Sector Onboarding'].map((step, i) => (
          ...
          <div className="lg:col-span-4 space-y-8 sticky top-24">
            <Card className="border-none shadow-lg rounded-[3rem] overflow-hidden bg-primary text-white p-10 relative">
              ...
              <div className="relative z-10 space-y-8">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/50 flex items-center gap-2">
                    <Target className="h-3 w-3 text-white/50" /> Evaluation Priority
                  </p>
                  <h3 className="text-2xl font-black italic tracking-tighter">High Intensity</h3>
                </div>
                <div className="pt-4">
                  <Button className="w-full h-16 rounded-2xl bg-white text-primary hover:bg-slate-50 font-black text-lg gap-2 shadow-xl shadow-black/10" onClick={handleApply}>
                    INITIALIZE JOURNEY <ChevronRight className="h-6 w-6" />
                  </Button>
                </div>
            ...
            <Card className="border-slate-200 shadow-sm rounded-[2.5rem] bg-white p-8 border">
              <div className="space-y-6">
                <h3 className="font-black text-sm uppercase tracking-widest flex items-center gap-2 text-slate-900">
                  <Star className="h-4 w-4 text-amber-500 fill-amber-500" /> Platform Insight
                </h3>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-400">Hiring Velocity</span>
                    <span className="text-xs font-black text-emerald-600">Accelerated</span>
                  </div>
                </div>
