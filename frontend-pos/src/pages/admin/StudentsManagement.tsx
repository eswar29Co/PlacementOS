...
export default function StudentsManagement() {
  ...
  const headerStats = [
    ...
    { label: 'Success Rate', value: '88%', icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-50/5' },
    { label: 'Active Applications', value: studentsList.reduce((sum: number, s: any) => sum + getStudentStats(s._id || s.id).active, 0), icon: Target, color: 'text-amber-600', bg: 'bg-amber-50/5' },
  ...
  return (
    <DashboardLayout title="Student Management" subtitle="View and manage all registered students">
          ...
          <CardHeader className="bg-slate-50 p-12 border-b border-slate-100">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
              <div className="space-y-2">
                ...
                <CardDescription className="font-bold text-[10px] uppercase tracking-[0.2em] text-slate-400">Manage student profiles and track their application progress</CardDescription>
              </div>
              ...
              <Table>
                <TableHeader className="bg-slate-50 h-20">
                  <TableRow className="hover:bg-transparent border-slate-100">
                    <TableHead className="px-12 font-black text-[10px] uppercase tracking-[0.2em] text-slate-400">Student Name</TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400">Education</TableHead>
                    ...
                    <TableRow className="border-none hover:bg-transparent">
                      <TableCell colSpan={4} className="py-48 text-center">
                        <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-700">
                          ...
                          <p className="font-black text-2xl text-slate-300 uppercase tracking-tighter italic">No Student Profiles Found</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
                        ...
                        <TableRow key={student._id || student.id} className="group hover:bg-slate-50/50 transition-all duration-300 border-slate-100">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.2em]">
                                  <span className={cn(
                                    ...
                                    {stats.offers > 0 ? 'PLACED' : stats.active > 0 ? `${stats.active} ACTIVE APPS` : 'NO APPLICATIONS'}
                                  </span>
                                  {stats.total > 0 && <span className="text-slate-500">{stats.total} APPLICATIONS</span>}
                                </div>
                              </div>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="px-12 text-right">
                            <div className="flex justify-end items-center gap-4">
                              {student.resumeUrl && (
                                <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-all border border-slate-200 shadow-sm" asChild title="View Resume">
                              ...
                              {student.linkedinUrl && (
                                <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-slate-50 hover:bg-blue-50 text-slate-400 hover:text-blue-500 transition-all border border-slate-200 shadow-sm" asChild title="LinkedIn Profile">
